...
public interface ClothingItemRepository extends JpaRepository<ClothingItem, UUID> {
    ...
    boolean existsByUserIdAndUploadHashAndRemovedAtIsNull(UUID userId, String uploadHash);
    boolean existsByUserIdAndUploadHashAndRemovedAtIsNullAndIdNot(
            UUID userId, String uploadHash, UUID id);
    ...
    List<ClothingItem> findByDuplicateOfIdAndStatus(
            UUID duplicateOfId, Enums.ProcessingStatus status);
