...
public class ImageAccessService {
    ...
    public String urlFor(ClothingItem item) {
        String objectKey = displayKey(item);
    ...
    private String displayKey(ClothingItem item) {
        if (item.getProcessedS3Key() != null && !item.getProcessedS3Key().isBlank()) {
            return item.getProcessedS3Key();
        }
        if (item.getOriginalS3Key() != null
                && !item.getOriginalS3Key().isBlank()
                && item.getOriginalDeletedAt() == null) {
            return item.getOriginalS3Key();
        }
        return null;
    }
}
