...
flowchart LR
  user[User browser]
  frontend[Vercel frontend<br/>React + TypeScript]
  auth[Cognito hosted auth]
  nginx[nginx reverse proxy<br/>EC2]
  api[Spring Boot API<br/>Docker container]
  db[(Postgres<br/>Docker + EBS volume)]
  images[(Private S3 image bucket)]
  queue[SQS image-processing queue]
  dlq[SQS dead-letter queue]
  worker[Python image worker<br/>Docker container]
  rembg[rembg background removal]
  gemini[Gemini vision API]
  backups[(S3 backup bucket)]
  backupJob[Postgres backup job]
  sched[Spring scheduled jobs]
  ...
  nginx --> api
  api --> db
  api -->|presigned PUT URL| images
  frontend -->|direct PUT original image| images
  images -->|ObjectCreated: users/*/original/*| queue
  queue --> worker
  queue -. repeated failures .-> dlq
  worker -->|claim/update item rows| db
  worker -->|read original, write processed, delete originals| images
  worker --> rembg
  worker --> gemini
  sched --> db
  sched -->|delete old originals / purged objects| images
  backupJob --> db
  backupJob --> backups
```

## Upload Processing State Diagram

```mermaid
stateDiagram-v2
  [*] --> WAITING_FOR_UPLOAD: API creates item + presigned URL
  WAITING_FOR_UPLOAD --> FAILED: browser reports original upload failure
  WAITING_FOR_UPLOAD --> FAILED: upload URL expires before S3 object appears
  WAITING_FOR_UPLOAD --> PROCESSING: worker claims original S3 event
  WAITING_FOR_UPLOAD --> PROCESSING: worker recovery finds missed S3 object
  PROCESSING --> READY: processed WebP + metadata saved
  PROCESSING --> READY: rembg or processed S3 upload fallback uses original
  PROCESSING --> READY: Gemini failure continues with empty tags
  PROCESSING --> DUPLICATE_REJECTED: processed hash matches existing READY item
  PROCESSING --> READY: timeout recovery promotes original
  PROCESSING --> FAILED: timeout recovery has no original image
  READY --> [*]
  DUPLICATE_REJECTED --> [*]
  FAILED --> [*]
```
